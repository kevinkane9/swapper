...
class HttpOperations
{
  ...
  */
  public function downloadFile($location, $pollEvery, $statusUrl, $removeAfter = FALSE, $self = '')
  {
    ...
    if ($filePointer !== false)
    {
      ...
      try
      {
        ...
        if ($removeAfter !== FALSE)
        {
          ...
          if (empty($result[1]))
          {
            ...
            $result[2] = $newFile . '.zip';
          return $result;
        }
        return [TRUE, $message, $newFile . '.zip'];
      ...
      catch(Exception $e)
      {
        if (false !== strpos($e->getMessage(), 'Could not resolve host')) {
            goto beginAttempt;
        } elseif (false !== strpos($e->getMessage(), 'Empty reply from server')) {
            goto beginAttempt;
        } else {
    ...
    {
      return [FALSE, "Could not open '$fullLocation' to download into.", null];
    }
  }

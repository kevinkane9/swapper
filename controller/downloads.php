    ...
    switch ($action) {
        case 'get-data':
            $totalCount = Db::fetchColumn(
                'SELECT COUNT(*) AS `count` FROM `sap_download`',
                [],
                'count'
            );
            ...
            $query = 'SELECT d.*, DATE_FORMAT(d.`created_on`, "%b %e, %Y") AS `date`,
                        l.`title` AS `title`
                        FROM `sap_download` d
                   LEFT JOIN `sap_list_request` l ON d.`list_request_id` = l.`id`';
            ...
            if (!empty($_POST['search']['value']) && '' !== $_POST['search']['value']) {
                $query .= ' WHERE d.filename LIKE :filename OR l.`title` LIKE :title';
                $params['filename'] = '%' . $_POST['search']['value'] . '%';
                $params['title']    = '%' . $_POST['search']['value'] . '%';
            }
        ...
        case 'delete':
            ...
            unlink('upload/' . $download['created_on'] . '/' . $download['filtered']);
            unlink('upload/' . $download['created_on'] . '/' . $download['purged']);

            Db::query(
                'DELETE FROM `sap_download` WHERE `id` = :id LIMIT 1',
                ['id' => Route::uriParam('id')]
            );
...
$downloads = Db::fetchAll('SELECT * FROM sap_download ORDER BY id DESC');
